...
class AenaApiService extends ApiService {
    ...
    /**
     * Cancelar turno usando filavirtual
     */
    async cancelTurnoFilavirtual(turnoId) {
        if (!this.filavirtualToken) {
            return {
                success: false,
                error: 'Token de filavirtual no configurado'
        ...
        try {
            const response = await fetch(`${this.filavirtualBaseURL}/turnos/${turnoId}/cancelar`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${this.filavirtualToken}`,
            ...
            console.error('Error cancelando turno filavirtual:', error);
            return {
                success: false,
                error: error.message
            };
